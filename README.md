...
### タブ列
- 左端にシステムタブ `General`
- 中央にユーザー作成タブ（横スクロール対応）
- タブ列の下に横スクロールバー
- 右側操作エリアに `+` ボタン（常時表示）
- ユーザー作成タブ群の右端にシステムタブ `Trash`
...
- キュー内に無効なタイマーID（削除済み・Trash移動済みなど）が残っていても、表示前に自動除去されます。
- スリープ復帰後は、有効タイマーの状態を再評価し、期限超過があれば alert 対象として扱います。
- relative repeat を `Stop` したあと再開した場合も、次回満了時に alert が表示されます。

## 13. エラーメッセージ
- 赤いエラーメッセージの右端に小さな `×` が表示されます。
- `×` を押すとその時点のメッセージを非表示にできます。
- 新しいエラーが発生した場合は再表示されます。
